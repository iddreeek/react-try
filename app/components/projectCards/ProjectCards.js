...
export const ProjectCards = ({ title, src, desc, stack }) => {
    return (
        <main className=' w-full h-full flex items-center justify-center'>
            <div className='h-full w-[80%] flex flex-col justify-center items-center text-black'>
                ...
                <div className='h-[15%] p-4 text-lg text-green-500 lato flex justify-center items-center gap-2'>
                    {stack.map((s)=>(
                        <div key={s.ID} className=' border-2 border-green-400 rounded-lg px-4 text-green-400 shadow-inner shadow-green-400'>{s.Tech}</div>
                    ))}
                </div>
                <div className='h-[15%] p-4 text-lg text-green-500 lato'>
                    {desc}
                </div>
            </div>
        </main>
